...
| | |
|---|---|
| **Tool Registry `[P1]`** | Đăng ký tool: name + JSON schema + handler. Validate input trước handler; lỗi validate trả message agent-sửa-được. Tool v0.1: `exec` (→ sandbox), `read_file`, `write_file`, `web_fetch` (qua egress whitelist). Registry là nguồn sinh tool schema cho Provider và stub cho RPC. |
...
| **Đầu ra** | CLI: `harness traces list/get/follow` + `export`. `[P3]` analytics (`/usage`, `/insights` — mẫu Hermes) đọc từ chính span store, không thu thập thêm. **OTLP export là module tách rời, mặc định TẮT** — bật chỉ khi khách có collector on-prem. Logs JSON đi qua cùng bộ redact của Result Filters (không lộ secret). |
